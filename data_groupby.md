...
``` r
weather_df %>% 
  group_by(name, month) # ungroup() #get rid of group
```
...
weather_df %>% 
  ...
  janitor::tabyl(name, cold) #create a 2x2 tabble
```
...
![](data_groupby_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->
...
![](data_groupby_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
...
weather_df %>% 
  ...
  knitr::kable(digits = 1) #round to the nearest digit
```
...
![](data_groupby_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->
...
weather_df %>% 
  ...
  mutate(
    ...
    ) %>% # lag : make every tmax value to the next row
  summarize(
